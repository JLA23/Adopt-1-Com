		...
		try {
			...
			for (int i = 0; i < l.size(); i++) {
				if (l.get(i).isValide()) {
					...
					out.println("<p><button type='' id='yes' class='btn btn-success btn-lg'>"
							+ "<span class='glyphicon glyphicon-thumbs-up'></span>"
							+ "</button>");
					out.println("<button type='' id='no' class='btn btn-danger btn-lg'>"
							+ "<span class='glyphicon glyphicon-thumbs-down'></span>"
							+ "</button></p>");
					out.println("<h3>" + l.get(i).getPrenom() + " "
							+ l.get(i).getNom() + "</h3>");
					out.println(""
							+ l.get(i).getMetier()
							+ "<br>"
							...
							+ "<br>"
							+ l.get(i).getVille()
							+ "\n<br><br><a class=\"btn btn-warning btn-lg\" href=\"PageProfil?id="
							+ id + "\" role=\"button\">En savoir plus</a>");
					out.println("</div>");
					out.println("</div>");
				}
			}
